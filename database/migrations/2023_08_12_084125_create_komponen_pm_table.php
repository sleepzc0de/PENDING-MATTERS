...
return new class extends Migration
{
    ...
    public function up()
    {
        Schema::create('komponen_pm', function (Blueprint $table) {
            $table->id();
            $table->unsignedInteger('ID_PENDING_MATTERS_MULTI_KOMPONEN');
            $table->string('SS_KOMPONEN')->nullable();
            $table->string('KODE_SS_KOMPONEN')->nullable();
            $table->string('PENDING_MATTERS_KOMPONEN')->nullable();
            $table->text('DEFINISI_PENDING_MATTERS_KOMPONEN')->nullable();
            $table->text('FORMULA_PENDING_MATTERS_KOMPONEN')->nullable();
            $table->text('KOMPONEN_PENGUKURAN_KOMPONEN')->nullable();
            $table->text('PENJELASAN_PENDING_MATTERS_KOMPONEN_KOMPONEN')->nullable();
            ...
            $table->timestamps();
            $table->softDeletes();
            $table->foreign('ID_PENDING_MATTERS_MULTI_KOMPONEN')->references('id')->on('pending_matters')->onDelete('cascade');
        });
    }
    ...
    {
        Schema::dropIfExists('komponen_pm');
    }
};
